...
if (basename(dirname(__FILE__)) != 'xmpreader') {
    add_event_handler('init', 'xmpreader_error');

    function xmpreader_error() {
        global $page;
        $page['errors'][] = 'xmpreader folder name is incorrect, uninstall the plugin and rename it to "xmpreader"';
    }
...
add_event_handler(
        'format_exif_data',
        'xmpreader_add_xmp_tags'
);

add_event_handler('get_admin_plugin_menu_links', 'xmpreader_admin_menu');

function xmpreader_add_xmp_tags($exif, $filename) {

    $xmp_array = conf_get_param('xmpreader', array());
...
function xmpreader_admin_menu($menu) {
    array_push(
            $menu,
            array(
                'NAME' => 'XMP Simple Reader',
